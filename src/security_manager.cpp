...
bool SecurityManager::load_config(const YAML::Node &config) {
    tbox::tsp::LogAdapter::security().info("tsp.security.load_config", "加载证书及密钥配置信息");
...
bool SecurityManager::apply_certification() {
    ...
    tbox::tsp::LogAdapter::security().info("tsp.security.cert_download",
        std::string("从服务器下载证书: ") + response);
    json response_json = json::parse(response);
    if (response_json["code"] == 0) {
        tbox::tsp::LogAdapter::security().info("tsp.security.cert_saved",
            std::string("保存证书: ") + certification_path_);
    ...
    tbox::tsp::LogAdapter::security().error("tsp.security.cert_download_failed",
        std::string("从服务器下载证书失败: ") + response_json["message"].get<std::string>());
    return false;
}
...
bool SecurityManager::renew_certification() {
    ...
    tbox::tsp::LogAdapter::security().info("tsp.security.cert_renew",
        std::string("从服务器续期证书: ") + response);
    json response_json = json::parse(response);
    if (response_json["code"] == 0) {
        tbox::tsp::LogAdapter::security().info("tsp.security.cert_backup",
            std::string("备份证书: ") + certification_path_ + hwyz::Utils::get_current_date());
    ...
    tbox::tsp::LogAdapter::security().error("tsp.security.cert_renew_failed",
        std::string("从服务器续期证书失败: ") + response_json["message"].get<std::string>());
    return false;
}
...
bool SecurityManager::apply_communication_secret_key() {
    ...
    tbox::tsp::LogAdapter::security().info("tsp.security.sk_apply",
        std::string("从服务器申请通讯密钥: ") + response);
    json response_json = json::parse(response);
    if (response_json["code"] == 0 && !response_json["data"]["encryptedSk"].is_null()) {
        ...
        tbox::tsp::LogAdapter::security().info("tsp.security.sk_encrypted",
            std::string("加密通讯密钥 + IV: ") + encrypted_comm_sk_with_iv);
        ...
        tbox::tsp::LogAdapter::security().info("tsp.security.sk_saved",
            std::string("保存通讯密钥: ") + secret_key_path_);
        // TODO 将通讯密钥写入安全芯片，这里先写本地文件
        return hwyz::Utils::write_file(secret_key_path_, comm_sk);
    }
    tbox::tsp::LogAdapter::security().error("tsp.security.sk_apply_failed",
        std::string("从服务器申请通讯密钥失败: ") + response_json["message"].get<std::string>());
    return false;
}