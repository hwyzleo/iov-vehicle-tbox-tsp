...
bool SomeipFacadeImpl::initialize() {
    LogAdapter::someip_bridge().info("tsp.someip.initializing", "SomeipFacadeImpl 初始化");
...
bool SomeipFacadeImpl::start() {
    ...
    LogAdapter::someip_bridge().info("tsp.someip.starting", "SomeipFacadeImpl 启动");
    ...
    if (!server_->start(
        ...
        LogAdapter::someip_bridge().error("tsp.someip.ipc_start_failed", "IPC 服务器启动失败");
        return false;
    }
...
void SomeipFacadeImpl::stop() {
    LogAdapter::someip_bridge().info("tsp.someip.stopping", "SomeipFacadeImpl 停止");
    if (server_) {
        server_->stop();
    }
}
...
void SomeipFacadeImpl::on_report_software_inventory(
    ...
    LogAdapter::someip_bridge().info("tsp.someip.register_callback", "注册上行回调");
    inventory_callback_ = std::move(callback);
}

bool SomeipFacadeImpl::push_fota_command(const std::vector<uint8_t>& payload) {
    if (!server_ || !server_->is_running()) {
        LogAdapter::someip_bridge().warn("tsp.someip.not_connected", "未连接，推送失败");
        return false;
    }

    LogAdapter::someip_bridge().info("tsp.someip.push_fota_command", "推送 FOTA 命令", {
...
std::string SomeipFacadeImpl::handle_request(ipc::MethodId method, const std::string& params_json, int client_fd) {
    LogAdapter::someip_bridge().debug("tsp.someip.handle_request", "处理请求", {
        {"method", tbox::fw::log::FieldValue::makeInt(static_cast<int64_t>(method))},
    ...
    switch (method) {
        ...
        default:
            LogAdapter::someip_bridge().warn("tsp.someip.unknown_method", "未知方法", {
                {"method", tbox::fw::log::FieldValue::makeInt(static_cast<int64_t>(method))}
            });
...
void SomeipFacadeImpl::handle_client_disconnect(int client_fd) {
    LogAdapter::someip_bridge().info("tsp.someip.client_disconnected", "客户端断开", {
        {"client_fd", tbox::fw::log::FieldValue::makeInt(client_fd)}
    });
}
    ...
    try {
        ...
        // 调用上行回调
        {
            ...
            if (inventory_callback_) {
                ...
                LogAdapter::someip_bridge().warn("tsp.someip.no_callback", "无上行回调注册");
            }
        }

        return "{\"success\":true}";
    } catch (const std::exception& e) {
        LogAdapter::someip_bridge().error("tsp.someip.parse_snapshot_failed", "解析 snapshot 失败", {
