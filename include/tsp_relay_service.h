// TBOX-TSP-DSN-CR-005 §3, §12.1: TspRelayService 业务聚合。
//
// 整体持有 SubscriptionCatalog / SubscriptionStore / MqttSubscriptionRegistrar /
// FotaHandler（去重、节流、回执、下行投递），对 Dispatcher 暴露 FotaRelayInterface
// facade。Application 是进程级组合根，负责构造/注入/启停顺序；本聚合保持可独立测试。
//
// 边界（CR-005 §12.1）：
// - 强内聚业务组件不得分别上提到 Application。
// - 注册控制器/FotaHandler 持有注入的 MqttFacade& 窄接口，业务对象所有权归本聚合。
// - Dispatcher/EventPublisher 依赖本 facade，不访问 Application。
// - 下行有界队列留在 TspEventPublisher（IPC 层，Application 持有），作为等价业务聚合的
//   合理偏差（设计 §12.1 成员列表为实现建议）。
...
class TspRelayService : public FotaRelayInterface {
public:
    ...
    //   加载并校验订阅目录、构造 store/registrar、构造并初始化 FotaHandler。
    // catalog_node 为 tsp.subscriptions 序列。失败返回 false 并清理已构造对象。
    bool initializeLocal(const std::string& device_sn,
    ...
    // 进入 STOPPING：reject-only，拒绝新 FOTA 上行/状态变更/订阅者注册 (CR-005 §7.1)。
    void beginShutdown();

    // 停止业务：停新 publish/重试/timer、取消下行 callback、收敛 accepted/unknown、
    // 持久化允许恢复的 generation/dedup/receipt 状态。
    void stop();
    ...
    // ---- FotaRelayInterface（委托 fota_handler_；STOPPING 时拒绝新上行）----
...
private:
    ...
    std::unique_ptr<FotaHandler> fota_handler_;
